locals {
  ...
  # ## Calculating pod subnets based on the pod base cidr and desired network bits
  # pod_base_cidr_network_bits = tonumber(regex("[^/]*$", var.pod_base_cidr_block))
  # pod_new_bits_list          = [for n in range(0, local.availability_zones_number) : var.pod_cidr_network_bits - local.pod_base_cidr_network_bits]
  # pod_subnet_cidr_blocks     = cidrsubnets(var.pod_base_cidr_block, local.pod_new_bits_list...)

  # pod_subnets = [
  #   for i, sn in local.pod_subnet_cidr_blocks :
  #   {
  #     "cidr_block" = sn,
  ...
  #     "name"       = "${var.subnet_name_prefix}-PodSubnet-${element(keys(local.availability_zones), i % floor(i / length(local.availability_zones)))}-${i + 1}",
  #     "type"       = "pod"
  #   }
  # ]
